...
async function main() {
    ...
    if ((link.includes("/checkout") || link.includes("/billing") || link.includes("/fatturazione")) && checkout_mode == "Full Checkout") {
        if (status_login == "on")
            mainLoggedCheckoutPayment()
        else
            mainLoggedCheckoutPayment()
    } else if ((link.includes("/spedizione") || link.includes("/shipping")) && checkout_mode == "Full Checkout") {
        if (status_login == "on")
            mainLoggedCheckoutShipping()
        else
            mainUnLoggedCheckoutShipping()
    } else if (link.includes("/riepilogoordine") || link.includes("/ordersummary")) {
        mainSuccess()
    } else {
...
async function mainCheckout() {
    try {
        sendWebhooks()
        if (checkout_mode == "Full Checkout" && document.getElementsByClassName("b-utility-menu__link icon-account js-user-account b-utility-menu__link_logged")[0] != undefined) {
            document.location = fetch_link[country]["payment_checkout_location"]
        } else if (checkout_mode == "Full Checkout") {
            document.location = fetch_link[country]["shipping_checkout_location"]
        } else
...
async function mainUnLoggedCheckoutShipping() {
    try {
    ...
    } catch (error) { errorWebhook(error, "mainUnLoggedCheckoutShipping") }
}

async function mainLoggedCheckoutShipping() {
    try {
        // document.location = fetch_link[country]["payment_checkout_location"]
        document.getElementsByClassName("b-btn_fourth b-checkout__proceed js-cmp-inited js-cmp-button")[0].click()
    } catch (error) { errorWebhook(error, "mainLoggedCheckoutShipping") }
}

async function mainLoggedCheckoutPayment() {
    try {
